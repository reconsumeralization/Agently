...
Agently 4.1.3.7 加固由 `AgentExecution` 承载的 AgentTaskLoop 路径：goal 和
success criteria 保持在同一个 execution draft 上，`effort(...)` 控制有边界的
strategy depth，DAG-shaped steps 会回收到 verification evidence，Skills context
packs 可以服务 planner 和 DAG consumers，ActionRuntime release blockers 也在不改变
ownership 边界的前提下关闭。完整版本叙事见
...
- **AI 服务应该是运行时执行，不是 prompt glue** - 一次 Agent execution 可以声明候选 Actions、Skills、MCP 服务、Dynamic Task 规划、过程流和输出契约，并通过同一套运行时表面执行。阅读 [4.1.3.7 Release Notes](docs/cn/development/release-notes-4.1.3.7.md)、[Agent Auto Orchestration 示例](examples/agent_auto_orchestration/) 和 [Skills Executor 示例](examples/skills_executor/)。
...
Agently 4.1.3.7 推荐 `agently-devtools >=0.1.9,<0.2.0`。
...
- 当前 package version 是 `4.1.3.7`。
- 当前 release manifest 是 `compatibility/releases/4.1.3.7.json`。
