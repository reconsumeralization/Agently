...
Agently 4.1.3.8 完成由 `AgentExecution` 承载的 AgentTaskLoop 任务执行策略优化：
`auto` 策略由 policy 解析为 `flat` 或 `taskboard`，`.strategy(...)` 是显式执行形态
选择器，`effort(...)` 控制 reflection density，ACP 是按需加载的 Action 加
ExecutionResource 能力，强格式过程请求使用 `.output(...)` 并带 JSON dict 兜底诊断，
包内也提供公开 typing metadata。完整版本叙事见
...
- **AI 服务应该是运行时执行，不是 prompt glue** - 一次 Agent execution 可以声明候选 Actions、Skills、MCP 服务、Dynamic Task 规划、过程流和输出契约，并通过同一套运行时表面执行。阅读 [4.1.3.8 Release Notes](docs/cn/development/release-notes-4.1.3.8.md)、[Agent Auto Orchestration 示例](examples/agent_auto_orchestration/) 和 [Skills Executor 示例](examples/skills_executor/)。
...
Agently 4.1.3.8 推荐 `agently-devtools >=0.1.10,<0.2.0`。
...
- 当前 package version 是 `4.1.3.8`。
- 当前 release manifest 是 `compatibility/releases/4.1.3.8.json`。
