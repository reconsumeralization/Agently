...
Agently 4.1.4.2 是建立在 4.1.4 execution baseline 上的破坏式 owner-boundary
release。`AgentExecution` 继续作为 direct、flat、TaskBoard strategy 共用的公开 run
surface；`TaskContext` 负责任务信息，`TaskWorkspace` 负责任务文件，`RecordStore`
负责持久 records，`SkillLibrary` 负责不可变 Skill revision。TriggerFlow 同时增加
active sub-flow 的原生 frame-scoped control。阅读
[4.1.4.2 Release Notes](docs/cn/development/release-notes-4.1.4.2.md)、
...
Skills 是可复用的任务指导包。在 4.1.4.2 中，`SkillLibrary` 负责不可变的已安装
...
Runtime、ExecutionResource、TaskWorkspace、RecordStore 或 TriggerFlow 管理。
...
Agently 4.1.4.2 推荐 `agently-devtools >=0.1.10,<0.2.0`。
...
- 当前 package version 是 `4.1.4.2`。
- 当前 release manifest 是 `compatibility/releases/4.1.4.2.json`。
