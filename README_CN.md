...
Agently 4.1.3.6 让 `AgentExecution` 成为推荐的单次 run owner：quick prompt
链通过 `AgentExecutionResult` 消费，Result-first readers 覆盖
text/data/meta/streams/task refs，bounded task loop 返回 task-strategy
`AgentExecution` draft，并加固 OpenAI-compatible usage-only final chunk 的
stream completion。完整版本叙事见
...
- **AI 服务应该是运行时执行，不是 prompt glue** - 一次 Agent execution 可以声明候选 Actions、Skills、MCP 服务、Dynamic Task 规划、过程流和输出契约，并通过同一套运行时表面执行。阅读 [4.1.3.6 Release Notes](docs/cn/development/release-notes-4.1.3.6.md)、[Agent Auto Orchestration 示例](examples/agent_auto_orchestration/) 和 [Skills Executor 示例](examples/skills_executor/)。
...
Agently 4.1.3.6 推荐 `agently-devtools >=0.1.7,<0.2.0`。
...
- 当前 package version 是 `4.1.3.6`。
- 当前 release manifest 是 `compatibility/releases/4.1.3.6.json`。
