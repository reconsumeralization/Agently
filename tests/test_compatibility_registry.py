...
def test_in_development_manifest_is_registered_and_protocol_compatible():
    ...
    assert "legacy 4.1.2.x-style AgentExecution chain reuse" in in_development["notes"]
    ...
    assert "fresh AgentExecution draft" in execution_contract["contract"]
    assert "execution.input(...).start() reuse" in execution_contract["contract"]
    assert "Completed-execution reconfiguration is compatibility support" in execution_contract["compatibility_policy"]
