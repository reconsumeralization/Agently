...
def test_in_development_manifest_is_registered_and_protocol_compatible():
    ...
    assert "do not silently set task-loop max_iterations or AgentExecution hard limits" in task_loop_contract["contract"]
    ...
    assert "TaskDAG is no longer an AgentTaskLoop bounded-step strategy" in task_loop_contract["contract"]
