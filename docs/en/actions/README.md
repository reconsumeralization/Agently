...
3. [Programmatic Action Calling](programmatic-action-calling.md): bounded, data-dependent read-only Action work through the `programmatic` planning protocol.
4. [ExecutionResource](execution-environment.md): advanced managed MCP/sandbox/process/browser/SQLite resources for Action, TriggerFlow, and plugin authors.
5. [Tools Compatibility](tools.md): old `tool_func` / `use_tools` aliases and their current status.
6. [MCP](mcp.md): mounting MCP servers as actions.
