...
`agently-stage >=0.3.8,<0.4.0`. It includes the Agently integration for the
...
[Agently-Stage #25](https://github.com/AgentEra/Agently-Stage/issues/25), plus
the Python 3.14 task-factory compatibility fix from
[Agently-Stage #28](https://github.com/AgentEra/Agently-Stage/issues/28).
...
the incomplete 0.3.6 routing fix. Stage 0.3.8 preserves the routing contract and
forwards keyword arguments such as `name` from Python 3.14's event-loop task
factory into `asyncio.Task`. The 4.1.4.7 development line adds end-to-end
TriggerFlow regression contracts and pins the minimum fully validated Stage
release.
...
| Stage dependency | Minimum version is `0.3.8` in the existing `<0.4.0` compatibility line | Adds Python 3.14 task-factory compatibility while retaining the 0.3.7 carrier-routing correction |
| Companion governance | Stage is the required-runtime companion; it publishes and passes Agently consumer validation before Agently raises its minimum | No lockstep versioning and no new Agently runtime facade |
...
Stage to 0.3.8. Install Agently 4.1.4.7 normally after that integration version
is released.
