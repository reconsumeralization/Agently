...
| Strategy verification | Flat and TaskBoard both use consumer-driven sufficiency for intermediate work units. Flat steps can return `ready_for_final_verification=false` with `remaining_work` so the next iteration consumes the facts instead of triggering an immediate verifier; TaskBoard downstream cards decide whether dependency evidence is enough for their objective. | Treat independent verifier requests as terminal, fan-in/control, trusted-boundary, contradiction, or high-risk review points. | This removes redundant after-action verifier calls without removing final acceptance or trusted Workspace/source/readback guards. |
