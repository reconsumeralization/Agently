...
| Scoped Workspace retrieval | Flat steps can carry `scoped_retrieval.query_groups`; the Flat BlockCarrier lowers those groups to pre-step Blocks `workspace_operation.search` facts and injects `scoped_retrieval_results` into the bounded `agent_step`. Query groups can choose `workspace_index`, `workspace_files`, or `workspace_index_and_files`, and Workspace search supports structural `collection`, `kind`, `id`, `path`, `scope`, and `meta` filters. Workspace file search accepts recursive `pattern="**"` as all files under the scoped path. TaskBoard source-ref policy exposes the same retrieval contract. | Search scoped Workspace/file evidence before broad reads when that reduces prompt input, then let the model judge whether snippets are useful. | Search hits are not local semantic acceptance, quality gates, or completion evidence. Flat file-surface retrieval has paired hot-context comparison evidence; TaskBoard strategy use still requires real-run effect evidence. |
