...
| TaskBoard path | TaskBoard no longer classifies complexity. It only runs after strategy policy selects it and preserves save/load/resume, handler diagnostics, card evidence contracts, and consumer-driven continuation. | Treat TaskBoard as a substrate for branched or multi-perspective work after policy selection. Let downstream cards decide whether upstream evidence is enough for their own objective. | TaskBoard is a board/dependency/patch coordinator, not a separate execution carrier. |
