...
3. [程序化 Action 调用](programmatic-action-calling.md)：通过 `programmatic` 规划协议完成有边界、数据依赖、只读的 Action 作业。
4. [ExecutionResource](execution-environment.md)：面向 Action、TriggerFlow 与插件开发者的高级托管 MCP/sandbox/process/browser/SQLite 资源。
5. [工具兼容](tools.md)：旧 `tool_func` / `use_tools` 别名的现状。
6. [MCP](mcp.md)：把 MCP server 挂成 action。
