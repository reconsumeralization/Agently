...
| Agent Component helpers | `enable_python`、`enable_shell`，以及通过 `enable_task_workspace_file_actions` 暴露当前 TaskWorkspace 文件作业区等业务快捷入口 | Provider 生命周期内部细节 |
...
- `ActionExecutor`：本地函数、MCP、Search/Browse、Bash、provider-neutral CodeExecution、SQLite、Docker
- `ExecutionResourceProvider`：ACP、MCP、Bash、Docker、显式无防护 trusted-local CodeExecution、Browser、SQLite
