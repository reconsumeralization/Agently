...
| `python` | `sandbox="trusted_local"` Python actions | 配置后的 in-process Python sandbox |
| `node` | `sandbox="trusted_local"` Node.js actions | 配置后的本地 Node.js runner |
| `docker` | 默认 `agent.enable_python(...)`、`agent.enable_shell(...)`、`agent.enable_nodejs(...)`、`agent.enable_code_runtime(...)` 以及 Docker executor actions | Docker CLI runner、镜像 provisioning 和语言 runtime profiles |
...
这些 provider 是低层环境实现。面向用户的能力通常应该暴露为 Action，场景快捷入口应该通过 Agent Component 或未来的 `agent.enable_*` helpers 暴露。Python、shell、Node.js 和常用语言 code runtime helpers 默认使用 Docker-backed runtime profiles；Docker CLI 或 daemon 预检查失败时 fail closed。strict profile 会报告缺失镜像而不是隐式拉取；developer 和 CI profile 可把缺失镜像拉取和标准依赖准备作为 host 选定的 provisioning 工作。显式 `sandbox="trusted_local"` 才会使用旧的本地 provider 兼容路径。
