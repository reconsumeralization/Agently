...
`acp_list_agents` 和 `acp_run_task`。内置 adapter 会检查常见 Codex 和 Claude Code
命令位置以及当前进程 `PATH`，使用框架固定 argv 模板，不暴露模型可见 shell execution。
...
默认 `agent.use_acp(session_scope="execution")` 会在同一个 AgentExecution context、
ACP agent id 和 root 内复用同一个框架 session descriptor；如果当前没有
AgentExecution context，会降级为 action-call session 并记录 diagnostics，而不是跨无关
host 调用共享状态。CLI adapter 会标记 `acp_session.persistence="stateless_cli"`，除非
存在真实可恢复的 ACP protocol session。
