...
如果省略 `root`，`agent.use_acp()` 使用当前 Agent 绑定的 Workspace `root`
...
id 与 kind。Workspace readback 不能满足指定 Action：一个可读文件可以证明文件内容，
...
Workspace patch，Flat 与 TaskBoard 都遵守这条路径。patch proposal 来自结构化
...
readback 并采用这一份确切的 Workspace 文件，不会在终态 materialization 中再用模型返回
...
execution flow、ActionFlow 与 TaskDAG execution 不绑定 Workspace；只有 `TriggerFlowActionFlow` 的 approval pause
只有 approval pause 需要 save/resume 时，才会绑定 Workspace recovery。
...
不能在 terminal cleanup 后继续创建 artifact 或 Workspace process record。child
