...
默认 `Browse()` 会依次尝试 Jina Reader、Playwright、BS4、restricted curl。Jina
Reader 会把目标 public URL 交给 `https://r.jina.ai/`，并消费其 URL-to-Markdown
输出；当默认 Reader endpoint 出现传输或服务错误时，Browse 会先尝试官方替代 endpoint
`https://r.jinaai.cn/`，再进入本地 backend 降级。如果应用不能接受这个外部 Reader
服务边界，可以显式关闭：
...
Browse 默认 backend 顺序是 Jina Reader -> Playwright -> BS4 -> restricted curl。
curl backend 只接收 Browse 规范化后的 URL candidate，不会作为 shell action 暴露给模型。
Jina Reader backend 只接收目标 URL 和 Reader 专用 headers；它是第三方页面读取 backend，
不是 Search 替代。Browse 会自动跳过传输错误、HTTP 5xx、空内容、明确 block/captcha/error
页，但不会判断模糊页面文本在语义上是否足够完成任务。
