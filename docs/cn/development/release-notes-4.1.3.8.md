...
| Scoped Workspace retrieval | Flat step 可以携带 `scoped_retrieval.query_groups`；TaskBoard source-ref policy 暴露同一个 retrieval contract。`search_files` 和 Blocks `workspace_operation.search/read_bounded` 返回有边界的事实型 `locator_ref` 与 `evidence_snippet`。 | 当 scoped Workspace/file evidence 能减少 prompt 输入时，先 search 再让模型判断 snippet 是否有用。 | 搜索命中不是本地语义验收、质量 gate 或完成证据。 |
