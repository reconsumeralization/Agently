...
| TaskBoard 路径 | TaskBoard 不再做复杂度分类；只有策略层选择它之后才执行，并保留 save/load/resume、handler diagnostics、card evidence contracts 和 consumer-driven continuation。 | 把 TaskBoard 当作分支或多视角任务的执行 substrate；由真正消费上游证据的下游 card 判断信息是否足够完成自己的目标。 | 独立 verifier 保留给终局验收、证据/artifact 边界审计、矛盾或高风险复核，不是每个行动后的必选步骤。 |
