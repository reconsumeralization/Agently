...
| Scoped Workspace retrieval | Flat step 可以携带 `scoped_retrieval.query_groups`；Flat BlockCarrier 会把这些 query groups 降到前置 Blocks `workspace_operation.search` 事实，并把 `scoped_retrieval_results` 注入有界 `agent_step`。query group 可以选择 `workspace_index`、`workspace_files` 或 `workspace_index_and_files`，Workspace search 支持 `collection`、`kind`、`id`、`path`、`scope`、`meta` 结构化过滤。Workspace file search 接受递归 `pattern="**"` 表示 scoped path 下所有文件。TaskBoard source-ref policy 暴露同一个 retrieval contract。 | 当 scoped Workspace/file evidence 能减少 prompt 输入时，先 search 再让模型判断 snippet 是否有用。 | 搜索命中不是本地语义验收、质量 gate 或完成证据。Flat 文件面 retrieval 已有成对 hot-context 对照证据；TaskBoard 策略使用仍需要真实运行效果证据。 |
