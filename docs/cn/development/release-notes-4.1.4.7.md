...
`agently-stage >=0.3.8,<0.4.0`，包含针对
...
0.3.6 中不完整的路由修正。Stage 0.3.8 保留该路由合同，并把 Python 3.14 event loop
task factory 提供的 `name` 等关键字参数继续传给 `asyncio.Task`。4.1.4.7 开发线增加
完整 TriggerFlow 链路的回归契约，并锁定已完整验证的最低 Stage 版本。
...
| Stage 依赖 | 在既有 `<0.4.0` 兼容线内将最低版本提升为 `0.3.8` | 增加 Python 3.14 task factory 兼容，同时保留 0.3.7 carrier 路由修复 |
| 伴随仓治理 | Stage 是必需运行时伴随仓；提高 Agently 最低版本前必须先发布 Stage 并通过 Agently 消费者校验 | 不采用版本号同步，也不新增 Agently runtime facade |
...
Agently 安装已经允许 Stage 0.3 兼容线，可以只把 Stage 升级到 0.3.8；待该集成版本
