...
description: Agently 4.1.4 关于 TaskBoard 增量验收、verifier cache 复用、scoped evidence 和进度 telemetry 的开发线说明。
...
Agently 4.1.4 是 4.1.3.9 之后的当前开发线。本页记录已经落地的
in-development 行为。
...
| Scoped verifier evidence | dirty final verification 收到 scoped evidence projection，只包含 dirty acceptance item 的有界 snippet。完整 SHA、bytes 和 raw body 继续留在 EvidenceEnvelope 与 Workspace 冷记录。 | verifier prompt 变小，但不替代 canonical evidence ledger。 | TaskBoard contract 测试和 2026-07-07 R14 四案例 DeepSeek 实测。 |
...
| Final response 与降级状态 | AgentTask 终局 result 现在为 accepted、degraded、partial、blocked outcome 都提供面向用户的 `final_response`。Flat verifier 终局优先使用同一次 verifier 请求里的字段，否则从结构化状态、artifact refs 和未满足项确定性生成说明；TaskBoard finalizer 继续使用同一次 finalizer 字段并合并 completion notes。`get_text()` / `async_get_text()` 对 task-strategy result dict 会优先返回 `final_response`。 | 增量 result 字段。`final_response` 和 `degraded` 是沟通/状态字段，不是完成证据；不会新增额外 narrator/model request。 | `tests/test_agent_execution_step_contract.py`、`tests/test_agent_task_loop.py`。 |
| Prompt-safe runtime context | AgentTask 仍会为诊断记录 runtime context，但默认 Flat/TaskBoard model-hot prompt 不携带具体运行时 timestamp，只暴露 prompt-safe 的可用性元信息。current/latest/as-of 业务日期必须来自 caller 输入或 source evidence。 | 防止运行日期泄漏成 unsupported artifact fact，同时保留模型热路径之外的运行诊断。 | `tests/test_agent_execution_step_contract.py` 和 R14 artifact inspection。 |
...
| Workspace 写入入口 | `workspace.put(...)` 现在是推荐的 record 写入 API，并支持 `content=...` 和 `profile=...` 来承接兼容 profile handler。`workspace.ingest(...)` 仅作为 `put(..., profile=...)` 的兼容 alias 保留；docs、Skills guidance 和 concept registry 不再把它提升为独立公共术语。 | 既有 `ingest(...)` 调用和已注册 profile handler 继续可用。新代码应调用 `put(...)`；Blocks `workspace_operation` 会把旧 `operation="ingest"` 规范化为 `operation="put"` 并带兼容标记。 | `tests/test_cores/test_workspace.py`、`tests/test_cores/test_blocks_plugin.py`。 |
