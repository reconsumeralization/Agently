...
| Dynamic DAG work | 使用 TaskDAG 处理 acyclic dynamic planning 和 execution。默认 `TaskDAGExecutor.async_run(...)` 直接编译到 TriggerFlow；只有需要 block-graph evidence/result mapping 时才显式选择 Blocks。 | `TaskDAGExecutor.async_run(...)`；可选 `TaskDAGExecutor.compile_blocks(...)` / `async_run_blocks(...)` |
