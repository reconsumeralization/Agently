...
Foundation 层能力是 release-critical substrate。当某个 release 触及或声称一个已经在
架构 spec 中定位为 Foundation 的能力时，仅有测试通过还不够：release reviewer 还必须
运行 `examples/` 下对应的核心 example，确认真实效果仍然能通过推荐 public API 跑通。
