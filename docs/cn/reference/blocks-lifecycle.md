...
外层 AgentTask 生命周期是一张 TriggerFlow 图：
...
阶段事件只传递 task/frame/version id，以及当前阶段的 plan、work-result 或 evidence id；
完整值留在其所属 request、Workspace object 或 host frame 中。TaskBoard 是
`work.execute` 内部的 work-producer subflow；它返回 iteration result 后，外层图直接进入
共享 transition，不会重复执行 Flat 的终态责任。

有边界 work stage 内的 Blocks lowering 路径是：
...
-> 单一语义 terminal verifier 与 host guards
```
...
绑定到已声明交付路径的成功文件 Action 拥有该路径的值边。output materialization 只能采用
它的物理 Workspace readback，不能把冲突的模型返回正文复制到同一路径。Action-owned 目标
无法读取时会阻塞交付；修改它必须执行新的文件 Action，从而产生新的 canonical Action
event 与 content version。

terminal verifier 只接收当前版本的 carrier inventory。它在同一个结构化 response 中返回
`criterion_checks`、`material_claim_coverage_complete` 与 `material_claim_checks`；
AgentTask host code 校验精确 criterion id、请求内 claim key 与 evidence reference id，
再在 host 侧恢复 carrier id、artifact quote、path 与 content version。Blocks 与 TaskBoard
不再运行第二套 claim inventory、source selection 或
support judgment 模型循环，也不拥有 accepted-result composition。
...
## 可选的 TaskDAG Through Blocks

默认 `TaskDAGExecutor.async_run(...)` 会校验 DAG 并直接编译到 TriggerFlow，
不会经过 Blocks。只有调用方需要 `ExecutionBlockGraph`、Blocks lifecycle evidence
或 evidence/result adapter 时，才显式选择 Blocks 路径。
