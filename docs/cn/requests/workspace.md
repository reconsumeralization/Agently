...
`agent.use_workspace(...)` 同时定义 `agent.workspace.files_root`，这是给 shell、
Node.js 和文件 action 使用的普通可编辑作业区。类文件系统的 Action helper 在没有
显式 root 或 cwd 时会继承这个边界。`agent.workspace.content_root` 仍然是 Workspace
records 使用的受管内容存储。
...
`enable_workspace_file_actions(...)` 不创建第二个 Workspace；它只是把当前 Workspace 文件作业区
暴露成 list/search/read/write 文件 actions。只有某个 action 必须使用独立目录时，
才显式传入 `root=` 或 `cwd=`。
