...
`workspace.search(...)` 仍作为兼容 alias 指向 `workspace.grep(...)`。

确定性文件检索使用 `workspace.grep_files(...)`。兼容方法
`workspace.search_files(...)` 调用同一条结构化文件检索路径：
...
- 向量检索通过 `method="vector"` 或 `method="hybrid"` 显式启用；
...
- 被选中的结构化 record payload 会渲染为确定性的 model-hot projection，并带上
  `original_ref` 和 `projection` 元数据；原始 Workspace record 仍是事实源，需要
  verification 时可以继续 readback；
