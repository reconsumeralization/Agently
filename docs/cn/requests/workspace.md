...
- 候选来源是 `DBStoreProvider` keyword/FTS 检索加 tag 检索；
...
- `method="auto"` 是默认值。只有 backend 同时存在 `EmbeddingProvider` 和
  `VectorStoreProvider`，并且 Workspace retrieval settings 表达了向量偏好时，才会解析为
  hybrid，例如
...
- 默认 local backend 使用 `db_store_provider="sqlite"` 保存 record DB，并使用
  `vector_store_provider="auto"`：`chromadb` 可导入且初始化成功时使用 Chroma，否则降级到
  `workspace.db` 内的 SQLite 向量表；
- record DB、embedding、向量存储是三个独立 provider。能力较低的
  `DBStoreProvider` 保持同一方法面，对不支持的高级能力返回空值或缺省值。vector store
  可以存在但没有 embedding provider；这种情况下向量模式会降级到确定性候选，并记录
  `embedding_provider_unavailable` 诊断。内置 `LocalVectorIndex(embedder)` 仍作为兼容
  adapter 保留，供旧代码继续用一个对象同时拥有向量化和本地向量打分；
...
task-scoped terminal cleanup 会按 scoped checkpoint record identity 关联 checkpoint
rows，而不是只看父 lifecycle run id。task 与 resume run id 对应的
`checkpoint.latest.<run-id>` manifest 会和被删除 records 一起清理；recovery anchor
则保留它指向的 compact resume checkpoint 及对应 latest manifest。
...
`supports_physical_reclamation`、`supports_remote_backend` 等 capability flags。
内置 local SQLite backend 只会在能够精确测量文件系统分配块并安全执行维护时报告物理回收能力。
Retention accounting 只测量 SQLite DB/WAL/SHM 文件；稳定的 Workspace mutation-lock
文件属于固定系统开销，绝不会被报告为 reclaimed bytes。分布式恢复应在所选 provider
...
Workspace 暴露 content、DB storage、policy、embedding 和 vector storage 等 provider
seam。默认本地 backend 使用 filesystem content，加 `db_store_provider="sqlite"` 管理
records/FTS/links/checkpoints/runtime metadata，并用 `vector_store_provider="auto"` 管理
向量存储：`chromadb` 可用时使用 Chroma，否则降级到 SQLite 向量表。`DBStoreProvider`
是归一的 record DB adapter surface：当前 SQLite、未来 MySQL/Postgres、以及本地文件
实现都走同一类 provider；低能力 provider 仍保留同一方法面，对不支持的高级能力返回空结果
或 absent value。Embedding 是独立的 `EmbeddingProvider`，向量存储是独立的
`VectorStoreProvider`，因此应用可以分别替换模型向量化、Chroma、SQLite 或自定义向量库。
内置 `LocalVectorIndex(embedder)` 仍作为兼容 adapter 保留，供旧代码继续用一个对象同时拥有
向量化和本地向量打分。
...
`examples/workspace/workspace_pluggable_vector_providers.py` 提供可运行的 provider
smoke：callable embedding provider 负责 records 的向量化索引，
`vector_store_provider="auto"` 在 Chroma 可用时使用 Chroma，否则降级到 SQLite
向量表。
...
不会复制 deliverable。在这个 child scope 中，`execution_id` 始终是继承的父
execution id，`task_id` 才是精确 Task 标识。因此 Task cleanup 只删除本 Task 的
process records/files，不影响父 execution 与 sibling Tasks。

AgentTask 失败时，终态清理仍会删除普通 process records、files 与 checkpoints。
如果上一轮已写入紧凑的 `task_id::resume` snapshot，Task 会在清理前增加 recovery
anchor，确保该恢复点可供显式 resume 使用。success 与 cancellation 默认不保留该恢复点。
...
file，并始终受 byte bound 约束。AgentExecution terminal finalizer 是
`agent_execution.completed`、`agent_execution.failed` 与
`agent_execution.cancelled` 的唯一 owner；内层 ModelRequest 完成只发送其自身的
request/model lifecycle event。

`AgentExecution.async_record_workspace(...)` 采用 purpose-aware lifecycle 规则。
`process` 与 `recovery` 只允许在 execution 正处于 running 时写入；该方法不会启动
fresh execution，也不会等待它完成。对应 checkpoint 始终使用 canonical
AgentExecution id。终态之后拒绝 `process` 与 `recovery` 写入；显式 `deliverable` 写入和 policy-enabled
`audit` 写入只有在立即接受 Workspace retention 治理时才允许。active recovery 或
lease facts 会推迟破坏性 cleanup，cancellation cleanup 同样如此。

Workspace 和可替换 provider 公开
`await workspace.get_retention_lifecycle(execution_id, status=...,
terminal_at=...)`，复用已有 typed lifecycle shape，从最新持久化 snapshot 读取
state version、未完成 recovery 与 active lease 事实。Agent consumer 直接转发这个
snapshot，不再合成 stream counter 或读取私有 runtime flag。取消使用独立的
`agent_execution.cancelled` RuntimeEvent 与 retention status。
