...
也不决定下一步要执行什么。默认 Agent 已经内置 lazy Workspace binding，因此标准
Agent facade 上始终可以访问 `agent.workspace`。默认 local backend 只会在代码第一次
写入、读取、checkpoint、记录 evidence 或暴露 Workspace 文件区时 materialize。
