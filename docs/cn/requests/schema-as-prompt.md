...
在严格的 API、SDK、模块或函数边界，description 是模型可见集成契约的一部分。
每个会被下游消费的叶子都应写明字段的契约含义，以及适用的枚举、格式、范围、
单位、可空性或字段依赖，不能只写 `"value"`、`"arguments"` 这类泛化标签。
同时通过 `info(...)` 提供权威接口文档；字段级契约细节属于必要的输出控制，
不是业务逻辑侵入。
