...
例如，会议行动项跟进产品的文档规划请求只负责内容覆盖与章节顺序，不负责写完整文档或
创建真实任务：

| 表达位置 | 说明性设计样例 |
|---|---|
| `input` | 用户提供的产品需求与尚未确定的问题。 |
| `info` | 文档读者、已确认范围与权威业务事实。 |
| `instruct` | 规划覆盖需求且不互相重复的章节；说明各节目的，缺少事实时提出问题而非编造。 |
| `output` | `parts: list[{title: str, brief: str}]`，parts/title/brief 非空；`open_questions: list[str]` 可为空列表。 |
| 消费者 | 写作请求消费章节计划，用户确认问题，宿主拥有顺序与组装身份。 |

按实际决策增减表格内容，必要时补充枚举、格式、范围、可空性和下游校验。这只是便于确认
的展示方式，不是固定模板、新 runtime schema，也不要求每次小改动都增加审批关卡。
代表性输入应标明性质并脱敏；展示必须与实际 chain/config 和渲染后的 Prompt 审计一致，
不能另维护一份容易漂移的 Prompt 真值。
