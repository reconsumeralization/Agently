...
不能仅因为一个案例暴露了失败，就把某个客户、器件或模型名称、页面状态、历史事故、
fixture 或已知答案对应的行为写成规范性 prompt 分支。应先定位缺失的通用不变量或决策
边界，写出覆盖该类问题的最小通用规则，再用原案例以及对照的正常、异常和边界案例验证。
除非这些字面值是当前请求真实提供的事实，否则应从规则中移除。
