...
当 Agently 开发者需要了解模型节点职责是否合理、调优执行效果、检查数据流是否正确完整、
是否存在缺失或冗余信息，以及模型与工程/工作流程分工是否恰当时，都可主动使用本方法；
不必明确说出“Prompt 审查”。方案设计和显式审查同样适用。应判断节点契约与上下游检查
是否确实能回答当前问题，而不是按关键词触发；无关的机械性修复不需要这套流程。

多轮推进时，每轮实质性回应先展示“状态在左、事项在右”的事项表，
以及含时间、讨论版本、变更原因、落实或放弃状态的变更表。
区分已确认、已落实与已验证，复用已有记录。参见
[多轮推进指导与样例](prompt-collaboration.md#多轮推进与变更记录)。

用户明确要求批量审阅、跳过细节或委托决定时，遵从该节奏。

审查复杂流程时，先展示从源输入到最终消费者的完整范围内流程图，高亮并标注模型节点
及其职责，区分宿主、Action/工具和人工工作。关键连线标注输入/输出，保留重要分支、
汇合、等待和循环。复用已有拓扑，区分实际行为、拟议变更和未确认的连线。

有可用的 imagegen 等生图工具或图表渲染工具、且能改善理解时，优先生成清晰的可视化图。
核对生成图中的节点、连线和职责是否与来源一致；Mermaid、SVG、ASCII 也是有效方案，
没有生图工具不能阻塞审查。精确契约仍用可读表格展示，不只藏在图片里。

邀请开发者校正清单和职责。范围明确时，流程总览与关联 Prompt 表格可以在同一回复中
展示，不强制先进行一轮只有清单的确认；有实质职责歧义时，先解决再设计依赖部分。
不多于三个逻辑模型节点/请求类型时，可以考虑一次分块展示全部 Prompt 表格；节点关联
紧密、合看更容易判断衔接时，可以超过三个。密集或大型流程按依赖和决策边界分组，
不机械按数量切割。重复执行的请求类型可以展示一次，但应说明变化输入和循环，不能
隐藏不同契约或实际调用次数。

分组依据是开发者偏好、关联性、信息量和决策风险，不是僵化的单节点或三节点上限。
展示分组不代表合并运行时请求。共享内容可以展示一次并明确各节点引用，但必须检查
它确实到达相应消费者；独立请求都需要的上下文不能仅因重复出现就被删除。

清单确认不等于新 Prompt 文本已获批准。清楚区分现有/拟议内容以及待确认/已确认状态；
除非开发者已委托决定，否则在有意义的分组边界确认重要变更后再实施。修订回到真实
chain/config，检查受影响的上下游，保留未变化的确认结果。

流程图和表格能暴露缺失输入、未消费字段或职责错误，但不能单独证明效果提升或根因。
这类结论仍需检查注入后的真实请求、输出和运行证据。不新增 runtime 协议，也不要求
所有常规请求和小改动都逐项审批。
...
流程总览之后，优先用表格展示各节点明细：请求总览、`Slot | 主题 | 实际 Prompt 内容` 主表、按需出现的
...
可参考[流程优先的协同设计样例](prompt-collaboration.md)。其中场景、请求清单和输出字段
仅用于演示呈现方式，不是强制业务模板；关联请求可以按上述分组与确认边界一起审阅。
