...
如果实测生成复杂度支持改用更浅的模型侧契约，可以由宿主显式映射回外部要求的完整结构。
外部 API/DTO 契约不变，真实调用前仍须校验；扁平化不能成为向下游传递残缺对象的理由。

## 复杂结构化输出实测不稳定时

不要仅因层级深就认定 schema 有问题。当代表性测试出现更多结构不遵循、丢 key、
ensure 失败、重试或长度失败时，先核对实际渲染 prompt、schema/格式支持、原始输出、
provider finish reason 与输出上限。上下文不足、parser 缺陷和 provider 截断需要不同修复。

可以按业务消费需要选择：

- 将冗余层级扁平化成精简的模型侧投影。例如仅返回 offered item key 与对应决策，
  不再让模型为每条结果抄写宿主已知的分组结构。决策所需的关系与事实仍须完整输入，
  再由宿主映射恢复完整结果。
- 在证据、输出规模、consumer 或独立校验/修复边界有实际价值时，拆成业务内聚的章节或
  有界批次。保留真实串行依赖，独立工作才采用有界并发。
- 分段结果和组装结果都按原始必要契约校验。不得为了通过率而删除 ensure、削弱硬规则，
  或把必填字段静默改为可选。

对照优化前后的结构/key 成功率、ensure 失败、重试与长度失败、请求数、延迟、真实 usage 和
语义覆盖。丢失业务含义的扁平化不算成功优化。
...
长文输出和大型结构化数据输出的生成目标不同；它们也都不同于截断后的交付恢复：
...
```text
需求与来源 -> 章节计划
  -> 本节写作 -> 校验并保存正文 -> 按需生成后续消费的摘要
  -> 宿主按计划顺序组装 -> 覆盖与跨章节一致性检查
```

写作请求应获得相关原始需求、本节标题与范围、必要的整体计划和来源事实，以及前文的
精简连续性信息。完整已接受正文保留在宿主存储或 artifact 中。摘要是有损派生上下文，
不能覆盖正式需求或替代精确来源证据；滚动上下文应有界，不要无限累积所有正文或摘要，
细节重要时再读回准确片段。

- 后文确实依赖前文已接受内容时，`for_each(concurrency=1)` 有合理依据。独立章节可以
  有界并发，由宿主按计划顺序收集；不要并发修改同一个滚动摘要列表。
- 不要求每节都单独发一次摘要请求。后续确实消费摘要，且同一响应内生成足够时，
  可以用浅层 `body` 在前、`continuity_note` 在后的一个请求；摘要需要反映校验、
  转换或读回后的正文时，才在这些结果到达后另发请求。正文与摘要取自同一个已接受结果，
  没有消费者的摘要应省略。
- 一节只产出一个自由文本成品时优先纯文本，确有消费者才增加结构化字段。使用独立请求
  draft 和异步 API；没有渐进消费者时直接 `async_get_data()` 即可。
- 宿主拼接已接受正文，不再让模型抄写整篇文档。检查章节覆盖与顺序，并对矛盾、重复、
  事实依据和可用性进行语义评审。局部修补缺口，明确报告失败章节，不靠灌水或字数宣称完成。
