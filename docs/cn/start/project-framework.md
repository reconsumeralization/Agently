...
对于具有独立 consumer、并行开发或局部验证价值的关键衔接点，可以在真实 producer
完成前提供精简、可替换的参考数据。这样下游可以围绕真正依赖的信息先行开发，上游也有
明确的消费目标。具体形式由项目决定，可以是 DTO、结构化样例、JSON fixture、event
payload、文件产物，或业务案例配合拓扑数据。

模型可以模拟专家生成初稿，但在开发者、领域事实、接口契约或真实运行确认前，这些内容
仍属于模拟材料。producer 完成后，必须证明真实输出能够替代参考数据；consumer 也不能
只适配某一个样例的文字或偶然取值。没有独立开发、回放、验证或故障定位价值时，不要增加
固定数据包 schema 或额外衔接层。

## 中间输出保持局部正确并推动任务

非终局的模型输出或助手回复，不需要在一轮内完成整个任务。先明确本轮所属阶段、负责的
主题、下一个 consumer 和局部验收边界。在这个范围内，输出仍必须满足 schema 与硬不变量，
忠实使用已提供事实，并避免已知的实质性逻辑错误或专业错误。

本轮输出应产生可观察的推进：推动一个状态变化、收窄一个不确定问题、选择一个可执行的
下一步，或生成后续阶段会消费的信息。假设、临时结论、未知项和延后工作必须明确标注，
不能冒充已经完成的事实；也不要为了显得完整而提前回答与本阶段无关的未来问题。

“推动任务”不能为局部契约错误或不安全动作开脱；反过来，也不能仅因后续工作尚未完成，
就否定一个边界清楚、局部正确的阶段产出。终局结果、整个任务完成声明和不可逆副作用仍须
通过完整的终局验收。例如，原理图带教助手可以在一轮中只给出并局部验收一个最小电路动作，
不必同时完成并认证整张原理图。
