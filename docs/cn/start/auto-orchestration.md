...
各自的 request、Workspace 或 host frame 中。每个 consumer 都会拒绝旧版本或跨任务
...
先构造一个带版本的 terminal-carrier inventory。Workspace carrier 由 host 签发的
...
Workspace artifact delivery 还会从实际解析出的 Markdown section headings 确定性生成
...
实际交付文件竞争。当 required Workspace path 已经存在时，它是唯一进入终态 verifier 的
Workspace carrier；其它可信 working files 只保留为冷侧 evidence，不再作为另一个终态
...
operation shape 复用 Workspace edit contract 的 `old_string` / `new_string` 字段，每个
...
Markdown emphasis，但真正应用补丁时，`old_string` 仍必须与 Workspace 原文精确匹配。
没有可信 Workspace candidate 的 inline candidate 仍使用有界 corrected-result 路径。
...
Workspace artifact 即使以相同内容复制到另一个 path 或 content version，仍是 transport
...
Workspace readback evidence 仍按原有规则参与验证。
...
对于 required Workspace deliverable，终态 finalization 以当前物理 locator/content-version
...
如果没有单独的显式答案，file-backed 终态才携带简洁 Workspace pointer；若 execution 在
...
Workspace。未知 carrier id、未知 evidence id，或不是当前 carrier 精确 span 的 quote 都会
...
Action，Workspace readback 也不能满足 Action requirement。如果能力不可用，repair 会
...
有范围的 Workspace readback 与审计，不再复制进每个热 summary。
...
`.strategy("taskboard")`。这个示例同时使用仓库源码资料 Action、Workspace 文件
...
对于 AgentTask route，required 远程 selector 会在业务规划前完成解析和安装。只有
发现、安装和检查都成功后，AgentTask 才会用规范化的已安装 Skill id 继续；否则
execution 会 fail closed 为 `blocked`，不会继续产出一个注定无法通过 required-Skill
门禁的答案。selector 级 `auto_allow=True` 只控制能力授权，不能把不可用的 Skill
变成可用。
