...
execution = agent.create_task(
    ...
    options={
        "agent_task": {
            ...
            # 可选：覆盖全局 agent_task.progress.language 设置。
...
`stream_kind="progress"` item 发出，便于日志和 UI 收敛状态。单次执行可用
`options={"agent_task": {"progress_language": "zh-CN"}}` 控制语言，也可以用
`Agently.set_settings("agent_task.progress.language", "zh-CN")` 设置全局默认；
`auto` 保持框架默认。主循环不会为了 progress 多产出字段，也不会等待 progress
