...
from ..Errors import RecordStoreError, RecordStorePolicyError
...
RECORD_IDENTITY_STATE_SCHEMA_VERSION = "record_identity_state/v1"
RECORD_IDENTITY_OBJECT_SCHEMA_VERSION = "record_identity_object/v1"
...
class RecordIdentityCatalog:
    """Private filesystem owner for one RecordStore identity high-water mark."""
    ...
    def __init__(
        ...
        *,
        ...
        if not str(record_store_id or "").strip():
            raise ValueError("RecordStore identity catalog requires a record_store_id.")
        self.system_root = Path(system_root).expanduser().resolve()
        self.root = self.system_root / "identity"
        self.record_store_id = str(record_store_id)
    ...
    def _allocate_locked(
        ...
        *,
        ...
        try:
            ...
            raise ValueError(f"Unknown RecordStore identity kind: {kind!r}.") from error
        state = self._read_state()
        sequence = int(state["high_water"]) + 1
        revision = int(state["revision"]) + 1
        identity = ScopedIdentity(
            scope_kind="record_store",
            scope_id=self.record_store_id,
            entity_id=f"{prefix}_{encode_base62(sequence)}",
            sequence=sequence,
        )
        manifest_path = self._manifest_path(kind, sequence)
        if manifest_path.exists():
            raise RecordStoreError("RecordStore identity state regressed to an already allocated sequence.")
        _write_json_atomic(
            self.state_path,
            {
                "schema_version": RECORD_IDENTITY_STATE_SCHEMA_VERSION,
        ...
        _write_json_atomic(
            manifest_path,
            {
                "schema_version": RECORD_IDENTITY_OBJECT_SCHEMA_VERSION,
    ...
    def _observe_content_sync(
        ...
        if not locator_kind or not normalized_locator:
            raise ValueError("RecordStore content observations require a locator kind and value.")
        if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            raise ValueError("RecordStore content observations require a SHA-256 digest.")
        if isinstance(size, bool) or not isinstance(size, int) or size < 0:
            raise ValueError("RecordStore content observation size must be a non-negative integer.")
        self._require_persistence()
        self.root.mkdir(parents=True, exist_ok=True)
        ...
        with _exclusive_file_lock(self.lock_path):
            ...
            if index_path.exists():
                index = self._read_json_object(index_path, label="RecordStore locator index")
                if (
                    index.get("record_store_id") != self.record_store_id
                    or index.get("locator_kind") != locator_kind
                    or index.get("normalized_locator") != normalized_locator
                ):
                    raise RecordStoreError("RecordStore locator index collision or owner mismatch.")
                ...
                index = {
                    "schema_version": "record_store_locator_index/v1",
                    "record_store_id": self.record_store_id,
                    "locator_kind": locator_kind,
                    "normalized_locator": normalized_locator,
            ...
            if not isinstance(versions, list):
                raise RecordStoreError("RecordStore locator version index is invalid.")
            for raw_version in versions:
                ...
                if not content_version_id:
                    raise RecordStoreError("RecordStore locator version index has an empty target.")
                ...
                if (
                    ...
                    raise RecordStoreError("RecordStore locator version target does not match its immutable manifest.")
                stored_size = raw_version.get("size")
                if isinstance(stored_size, bool) or not isinstance(stored_size, int) or stored_size != size:
                    raise RecordStoreError("RecordStore locator version digest matches but its size differs.")
    ...
    def _resolve_sync(self, entity_id: str) -> dict[str, Any]:
        ...
        if not path.exists():
            raise KeyError(f"RecordStore identity does not exist: {entity_id}")
        manifest = self._read_json_object(path, label="RecordStore identity object")
        if manifest.get("entity_id") != entity_id or manifest.get("scope_id") != self.record_store_id:
            raise RecordStoreError("RecordStore identity object does not match its scoped key.")
        return manifest
    ...
    def _add_segment_sync(
        ...
        for label, value in {"ordinal": ordinal, "offset": offset, "length": length}.items():
            if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                raise ValueError(f"RecordStore segment {label} must be a non-negative integer.")
        digest = str(digest or "").strip().lower()
        ...
        with _exclusive_file_lock(self.lock_path):
            ...
            if parent.get("kind") != "content_version":
                raise ValueError("RecordStore segments must belong to a content version.")
    ...
    def _add_link_sync(
        ...
        if not relation or not role:
            raise ValueError("RecordStore identity links require relation and role values.")
        self._require_persistence()
        self.root.mkdir(parents=True, exist_ok=True)
        with _exclusive_file_lock(self.lock_path):
    ...
    def _retain_task_manifest_sync(
        ...
        if not normalized_task_id:
            raise ValueError("RecordStore retained task manifests require a task_id.")
        if normalized_state not in {"active", "recovery", "accepted", "released"}:
            raise ValueError("RecordStore retained task manifest state is invalid.")
        ...
        if any(not entity_id for entity_id in normalized_roots):
            raise ValueError("RecordStore retained task roots cannot be empty.")
        self._require_persistence()
        self.root.mkdir(parents=True, exist_ok=True)
        with _exclusive_file_lock(self.lock_path):
            ...
            if task_path.exists():
                task_manifest = self._read_json_object(
                    task_path,
                    label="RecordStore task identity manifest",
                )
                if task_manifest.get("task_id") != normalized_task_id:
                    raise RecordStoreError("RecordStore task identity manifest has an invalid owner.")
            else:
                task_manifest = {
                    "schema_version": "record_store_task_identity_manifest/v1",
                    "record_store_id": self.record_store_id,
    ...
    def _pin_sync(self, entity_id: str, reason: str) -> None:
        ...
        if not normalized_reason:
            raise ValueError("RecordStore identity pins require a reason.")
        self._require_persistence()
        self.root.mkdir(parents=True, exist_ok=True)
        with _exclusive_file_lock(self.lock_path):
            self._resolve_sync(entity_id)
            ...
            _write_json_atomic(
                self.pins_path,
                {
                    "schema_version": "record_identity_pins/v1",
                    "record_store_id": self.record_store_id,
                    "pins": pins,
                },
            )
    ...
    def _collect_unreachable_sync(
        ...
        with _exclusive_file_lock(self.lock_path):
            ...
            for task_path in self.root.glob("tasks/*/manifest.json"):
                ...
                if task_manifest.get("record_store_id") != self.record_store_id:
                    raise RecordStoreError("RecordStore task identity manifest owner mismatch.")
                ...
                if not isinstance(raw_roots, list):
                    raise RecordStoreError("RecordStore task identity roots are invalid.")
            ...
            if unknown_roots:
                raise RecordStoreError(
                    "RecordStore identity retention roots are missing: " + ", ".join(sorted(unknown_roots))
                )
            ...
            for locator_id, index_path in locator_index_paths.items():
                ...
                if current_version_id not in retained:
                    raise RecordStoreError("Retained RecordStore locator lost its current content version.")
            ...
            for entity_id in sorted(deleted):
                ...
                if entity_id in audit_ids:
                    ...
                    _write_json_atomic(
                        ...
                        {
                            "schema_version": "record_identity_tombstone/v1",
                            "record_store_id": self.record_store_id,
                            "entity_id": entity_id,
    ...
    def _discard_sync(self, entity_ids: Sequence[str]) -> tuple[str, ...]:
        ...
        with _exclusive_file_lock(self.lock_path):
            for entity_id in normalized:
                ...
                manifest = self._read_json_object(path, label="RecordStore identity object")
                if manifest.get("entity_id") != entity_id or manifest.get("scope_id") != self.record_store_id:
                    raise RecordStoreError("RecordStore identity discard target is invalid.")
                path.unlink()
    ...
    def _read_pins(self) -> dict[str, str]:
        if not self.pins_path.exists():
            return {}
        raw = self._read_json_object(self.pins_path, label="RecordStore identity pins")
        if (
            raw.get("schema_version") != "record_identity_pins/v1"
            or raw.get("record_store_id") != self.record_store_id
            or not isinstance(raw.get("pins"), dict)
        ):
            raise RecordStoreError("RecordStore identity pins are invalid.")
    ...
    def _load_identity_manifests(self) -> tuple[dict[str, dict[str, Any]], dict[str, Path]]:
        ...
        for path in self.root.glob("objects/*/*/*.json"):
            ...
            if not entity_id or entity_id in manifests or manifest.get("scope_id") != self.record_store_id:
                raise RecordStoreError("RecordStore identity object key or owner is invalid.")
    ...
    def _load_locator_indexes(self) -> tuple[dict[str, dict[str, Any]], dict[str, Path]]:
        ...
        for path in self.root.glob("indexes/locators/*/*.json"):
            index = self._read_json_object(path, label="RecordStore locator index")
            locator_id = str(index.get("locator_id") or "")
            if not locator_id or locator_id in indexes or index.get("record_store_id") != self.record_store_id:
                raise RecordStoreError("RecordStore locator index key or owner is invalid.")
    ...
    def _validate_digest(digest: str) -> None:
        if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            raise ValueError("RecordStore content identities require a SHA-256 digest.")

    def _lease_task_range_sync(self, task_id: str, size: int) -> tuple[int, int]:
        normalized_task_id = str(task_id or "").strip()
        if not normalized_task_id:
            raise ValueError("RecordStore identity range leases require a task_id.")
        if isinstance(size, bool) or not isinstance(size, int) or size <= 0:
            raise ValueError("RecordStore identity range lease size must be a positive integer.")
        self._require_persistence()
        self.root.mkdir(parents=True, exist_ok=True)
        with _exclusive_file_lock(self.lock_path):
            state = self._read_state()
            ...
            if task_path.exists():
                try:
                    ...
                    raise RecordStoreError("RecordStore task identity manifest is corrupt.") from error
            ...
            if not isinstance(leases, list):
                raise RecordStoreError("RecordStore task identity leases are invalid.")
    ...
    def _require_persistence(self) -> None:
        if not self.private_write:
            raise RecordStorePolicyError("Agently-private RecordStore identity persistence is disabled.")
        if not self.create and not self.state_path.exists():
            raise RecordStorePolicyError("RecordStore identity persistence is not initialized and create=False.")
    ...
    def _read_state(self) -> dict[str, str | int]:
        ...
        try:
            ...
            raise RecordStoreError("RecordStore identity state is unreadable or corrupt.") from error
        if not isinstance(raw, Mapping):
            raise RecordStoreError("RecordStore identity state must be a JSON object.")
        if raw.get("schema_version") != RECORD_IDENTITY_STATE_SCHEMA_VERSION:
            raise RecordStoreError("RecordStore identity state schema is unsupported.")
        if raw.get("record_store_id") != self.record_store_id:
            raise RecordStoreError("RecordStore identity state belongs to a different RecordStore.")
        ...
        if (
            ...
            raise RecordStoreError("RecordStore identity state counters are invalid.")
        if int(high_water) < 0 or revision > int(high_water):
            raise RecordStoreError("RecordStore identity state counters regressed or diverged.")
    ...
    def _read_json_object(path: Path, *, label: str) -> dict[str, Any]:
        try:
            ...
            raise RecordStoreError(f"{label} is unreadable or corrupt.") from error
        if not isinstance(raw, dict):
            raise RecordStoreError(f"{label} must be a JSON object.")
        return dict(raw)
    ...
    def _parse_entity_id(entity_id: str) -> tuple[IdentityKind, int]:
        ...
        if not separator or not encoded:
            raise ValueError(f"Invalid RecordStore identity: {entity_id!r}.")
        ...
        try:
            ...
            raise ValueError(f"Unknown RecordStore identity prefix: {prefix!r}.") from error
        sequence = decode_base62(encoded)
        if sequence <= 0:
            raise ValueError("RecordStore entity ids cannot use the unallocated zero sentinel.")
